...
$sql = "SELECT nombre_curso, descripcion FROM cursos_premium WHERE id = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_update = "UPDATE cursos_premium SET descripcion = ? WHERE id = ?";
...
<div>
  ...
  <textarea id="descripcion" name="descripcion" rows="5" required
    class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent"><?php echo ($curso['descripcion']); ?></textarea>
</div>
...
<script>
  tinymce.init({
    selector: '#descripcion',
            plugins: 'lists link image preview anchor table', // Configuración de plugins

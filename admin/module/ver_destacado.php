...
$sql = "SELECT cg.id, cg.nombre_curso, cg.descripcion, cg.imagen_portada,
        ...
        LEFT JOIN archivos_pdf_destacados apg ON cg.id = apg.curso_id
        LEFT JOIN imagenes_curso_destacados icg ON cg.id = icg.curso_id
        LEFT JOIN videos_curso_destacados vcg ON cg.id = vcg.curso_id
        GROUP BY cg.id";
$result = $conn->query($sql);
?>
<div class="max-w-7xl mx-auto mt-10 p-5">
    <h2 class="text-3xl font-bold mb-6 text-center text-purple-700">Cursos Destacados</h2>
                        ...
                        <tr class="hover:bg-gray-200 transition duration-200">
                            ...
                            <td class="py-3 px-4 border">
                                ...
                                if (!empty($row['videos'])):
                                    ...
                                    $courseId = $row['id'];
                                    echo "<a href='./?page=ver_videos_destacados&curso_id=" . htmlspecialchars($courseId) . "' class='text-blue-500 hover:underline'>Ver Todos los Videos</a>";
                                else: ?>
                                    No disponible
                                <?php endif; ?>
                            </td>
                            ...
                            <td class="py-3 px-4 border text-center">
                                <a href="./?page=modificar_curso_titulo_destacado&id=<?php echo $row['id']; ?>"
                                ...
                                <a href="./?page=modificar_curso_categorias&id=<?php echo $row['id']; ?>&tipo=destacado"
                                    class="inline-block bg-yellow-400 text-gray-800 font-semibold py-1 px-4 rounded-full shadow-md hover:bg-yellow-500 hover:shadow-lg transition duration-200 ease-in-out transform hover:scale-105">
                                    Modificar Categorias
                                </a>
                                <a href="./?page=modificar_curso_descripcion_destacado&id=<?php echo $row['id']; ?>"
                                    class="inline-block bg-yellow-400 text-gray-800 font-semibold py-1 px-4 rounded-full shadow-md hover:bg-yellow-500 hover:shadow-lg transition duration-200 ease-in-out transform hover:scale-105">
                                    Modificar Descripción
                                </a>
                                <a href="./?page=modificar_curso_portada_destacado&id=<?php echo $row['id']; ?>"
                                    class="inline-block bg-yellow-400 text-gray-800 font-semibold py-1 px-4 rounded-full shadow-md hover:bg-yellow-500 hover:shadow-lg transition duration-200 ease-in-out transform hover:scale-105">
                                    Modificar Portada
                                </a>
                                <a href="./?page=modificar_curso_pdf_destacado&id=<?php echo $row['id']; ?>"
                                    class="inline-block bg-yellow-400 text-gray-800 font-semibold py-1 px-4 rounded-full shadow-md hover:bg-yellow-500 hover:shadow-lg transition duration-200 ease-in-out transform hover:scale-105">
                                    Modificar PDFs
                                </a>
                                <a href="./?page=modificar_curso_imagenes_destacado&id=<?php echo $row['id']; ?>"
                                    class="inline-block bg-yellow-400 text-gray-800 font-semibold py-1 px-4 rounded-full shadow-md hover:bg-yellow-500 hover:shadow-lg transition duration-200 ease-in-out transform hover:scale-105">
                                    Modificar Imágenes
                                </a>
                            </td>
                            ...
                            <!-- Botón Eliminar -->
                            <td class="py-3 px-4 border text-center">
                                <a href="./controllers/eliminar_curso_destacado.php?id=<?php echo $row['id']; ?>" class="inline-block bg-red-500 text-white font-bold py-1 px-3 rounded hover:bg-red-600 transition duration-200">Eliminar</a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                <?php else: ?>

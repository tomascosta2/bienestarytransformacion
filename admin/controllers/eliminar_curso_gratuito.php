...
if (isset($_GET['id'])) {
    ...
    $sql_imagenes = "SELECT ruta_imagen FROM imagenes_curso_gratuitos WHERE curso_id = ?";
    ...
    $sql_delete = "DELETE FROM cursos_gratuitos WHERE id = ?";
    $stmt_delete = $conn->prepare($sql_delete);
    $stmt_delete->bind_param("i", $curso_id);
    if ($stmt_delete->execute()) {
        // Eliminar archivos PDF y videos asociados
        $conn->query("DELETE FROM archivos_pdf_gratuitos WHERE curso_id = $curso_id");
        $conn->query("DELETE FROM videos_curso_gratuitos WHERE curso_id = $curso_id");
        
        // Eliminar imágenes del curso
        $conn->query("DELETE FROM imagenes_curso_gratuitos WHERE curso_id = $curso_id");
